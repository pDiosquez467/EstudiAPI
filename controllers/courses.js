...
class CourseController {
    ...
    async addStudent(req, res) {
        ...
        try {
            ...
            if (course.length === 0) {
                return res.status(404).json({ msg: `No course found with ID ${course_id}` })
            }
            ...
            if (student.length === 0) {
                return res.status(404).json({ msg: `No student found with ID ${student_id}` })
            }
       ...
       VALUES (?, ?)`,
            ...
            res.status(201).json({ msg: `Student ${student_id} added to course ${course_id}` })
